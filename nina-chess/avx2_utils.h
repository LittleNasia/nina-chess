...
#ifdef __AVX2__
forceinline float simd_horizontal_sum(const simd_vector& input)
{
	...
	__m128 lower_half = _mm256_extractf128_ps(input, 0);
	__m128 higher_half = _mm256_extractf128_ps(input, 1);

	// higher_half = [x0+x4, x1+x5, x2+x6, x3+x7]
	higher_half = _mm_add_ps(higher_half, lower_half);

	// lower_half = [x2+x6,x3+x7 ...] 
	lower_half = _mm_movehl_ps(higher_half, higher_half);
	...
	higher_half = _mm_add_ps(lower_half, higher_half);
	...
	lower_half = _mm_shuffle_ps(higher_half, higher_half, _MM_SHUFFLE(0, 0, 0, 1));

	// now we can just add the very first elements using a simple scalar add
	higher_half = _mm_add_ss(higher_half, lower_half);

	// return the scalar value 
	return _mm_cvtss_f32(higher_half);
}
#endif
